...
from rag_helpers import setEnvs  # Ensure this loads your env vars from rag.env
...
SLACK_CHANNEL = os.getenv("SLACK_CHANNEL", "#fraud_squad")

# --- Jira Ticket Bot with Dynamic Conversation ---
class JiraTicketBot:
    ...
    def dynamic_conversation(self, state: dict) -> dict:
        """
        Build a conversation context from the current state and let the LLM extract
        key details for your Jira ticket.
        """
        context_parts = []
...
# --- Streamlit App Initialization Functions ---
def init_state():
    ...
    if "ticket_bot" not in st.session_state:
        setEnvs(path="../rag.env")  # Load env vars from rag.env
        logger = logging.getLogger(__name__)
        llm = ChatOpenAI(
...
def main():
    ...
    # Helper: Check for "@fraud_noc" in user input.
    ...
    # --- Step 3: Confirm or add details (if not skipping details) ---
    ...
    if not st.session_state.ticket_submitted:
        with st.chat_message("assistant"):
            with st.spinner("Creating your ticket..."):
                ...
                inputs.update(conv_result)
                ticket_data = bot.confirm_ticket(inputs)
                # If skip_details flag is set, override ticket data with low-context defaults.
                if inputs.get("skip_details"):
                ...
                st.markdown(message)
                # Post the ticket message to Slack.
                try:
                    slack_client.chat_postMessage(
                        channel=SLACK_CHANNEL,
                        text=message
                    )
                    logger.info("Posted ticket info to Slack.")
                except Exception as slack_error:
                    logger.error(f"Error posting to Slack: {slack_error}")

    # --- Step 5: Final message and start over ---
